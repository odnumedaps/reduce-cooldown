...
local originals = setmetatable({}, {__mode = "k"}) -- weak keys
...
----------------------------------------------------------------
-- DELTA WARNING SYSTEM (from Script 1)
----------------------------------------------------------------
local deltaWarning = Instance.new("TextLabel")
deltaWarning.Size = UDim2.new(1, 0, 0, 30)
deltaWarning.Position = UDim2.new(0, 0, 1, -30)
deltaWarning.BackgroundColor3 = Color3.fromRGB(120, 0, 0)
deltaWarning.TextColor3 = Color3.new(1, 1, 1)
deltaWarning.Font = Enum.Font.GothamBold
deltaWarning.TextSize = 16
deltaWarning.Text = "⚠ Delta Spike Detected!"
deltaWarning.Visible = false
deltaWarning.Parent = main

local deltaThreshold = 0.08 -- 80ms per frame (lag/spike)
RunService.Heartbeat:Connect(function(dt)
    if dt > deltaThreshold then
        deltaWarning.Visible = true
        deltaWarning.Text = string.format("⚠ Delta Spike: %.3f", dt)
